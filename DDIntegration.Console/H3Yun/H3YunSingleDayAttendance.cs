    ...
    public class H3YunSingleDayAttendance
    {
        ...
        public static H3YunSingleDayAttendance ConvertFrom(List<H3YunAttendance> attendances, List<LeaveStatus> leaveStatus, string userId)
        {
            ...
            try
            {
                ...
                // 两次打卡制
                if (onDutyAttendances.Count == 1 && onDutyAttendances[0].F0000012.Hour < 11 &&
                    offDutyAttendances.Count == 1 && offDutyAttendances[0].F0000012.Hour > 14)
                {
                    result = GetTwiceCheckAttendance(attendances, onDutyAttendances, offDutyAttendances, weiDaKaAttends, leaveStatus);
                }
                // 四次打卡制
                else
                {
                    result = GetFourTimesCheckAttendance(attendances, onDutyAttendances, offDutyAttendances, weiDaKaAttends, leaveStatus);
                }

                result.F0000001 = userId;
                result.F0000002 = attendances[0].F0000006;
                result.F0000003 = attendances[0].F0000005;
                result.F0000009 = attendances[0].F0000005.ToString("yyyy-MM");
            }
            catch (Exception ex)
            {
            }
        ...
        private static H3YunSingleDayAttendance GetTwiceCheckAttendance(
            List<H3YunAttendance> attendances,
            List<H3YunAttendance> onDutyAttendances,
            List<H3YunAttendance> offDutyAttendances,
            List<H3YunAttendance> weiDaKaAttendances,
            List<LeaveStatus> leaveStatus)
        {
            H3YunSingleDayAttendance result = new H3YunSingleDayAttendance();

            DateTime onDutyTime = onDutyAttendances[0].F0000012;
            DateTime offDutyTime = offDutyAttendances[0].F0000012;

            // 请假一天
            if (leaveStatus.Where(s => s.starttime <= onDutyTime && s.endtime >= offDutyTime).Any())
            {
                return result;
            }
            ...
            // 下午请假
            if (leaveStatus.Where(s => s.starttime > onDutyTime && s.starttime <= onDutyTime.AddHours(4) && s.endtime >= offDutyTime).Any())
            {
                if (onDutyAttendances[0].F0000008 == "未打卡")
                {
                    result.F0000005 = 0.5;
                }
                return result;
            ...
            // 上班未打卡
            if(onDutyAttendances[0].F0000008 == "未打卡" || offDutyAttendances[0].F0000008 == "未打卡")
            {
                result.F0000005 = 0.5;
                result.F0000004 = 0.5;
                return result;
        ...
        private static H3YunSingleDayAttendance GetFourTimesCheckAttendance(
            List<H3YunAttendance> attendances,
            List<H3YunAttendance> onDutyAttendances,
            List<H3YunAttendance> offDutyAttendances,
            List<H3YunAttendance> weiDaKaAttends,
            List<LeaveStatus> leaveStatus)
        {
            H3YunSingleDayAttendance result = new H3YunSingleDayAttendance();
